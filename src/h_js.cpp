...
int Hilit_JS(EBuffer *BF, int /*LN*/, PCell B, int Pos, int Width, ELine *Line, hlState &State, hsState *StateMap, int *ECol) {
    int j = 0;
    int firstnw = 0;
...
static int JS_Indent = 4;
static int JS_BraceOfs = 0;
static int JS_ParenDelta = -1;
static int JS_CaseOfs = 0;
static int JS_CaseDelta = 4;
static int JS_ClassOfs = 0;
static int JS_ClassDelta = 4;
static int JS_ColonOfs = 0;//-4;
static int JS_CommentOfs = 0;
static int JS_CommentDelta = 1;
static int JS_FirstLevelWidth = -1;
static int JS_FirstLevelIndent = 4;
static int JS_Continuation = 4;
static int FunctionUsesContinuation = 0;

// this is global, unfortunately -- FIX !!!
int EBuffer::SetJSIndentStyle(ExState &State) {
    if (State.GetIntParam(View, &JS_Indent) == 0)
        return 0;
    if (State.GetIntParam(View, &JS_BraceOfs) == 0)
        return 0;
    if (State.GetIntParam(View, &JS_ParenDelta) == 0)
        return 0;
    if (State.GetIntParam(View, &JS_CaseOfs) == 0)
        return 0;
    if (State.GetIntParam(View, &JS_CaseDelta) == 0)
        return 0;
    if (State.GetIntParam(View, &JS_ClassOfs) == 0)
        return 0;
    if (State.GetIntParam(View, &JS_ClassDelta) == 0)
        return 0;
    if (State.GetIntParam(View, &JS_ColonOfs) == 0)
        return 0;
    if (State.GetIntParam(View, &JS_CommentOfs) == 0)
        return 0;
    if (State.GetIntParam(View, &JS_CommentDelta) == 0)
        return 0;
    if (State.GetIntParam(View, &JS_FirstLevelWidth) == 0)
        return 0;
    if (State.GetIntParam(View, &JS_FirstLevelIndent) == 0)
        return 0;
    if (State.GetIntParam(View, &JS_Continuation) == 0)
        return 0;
    return 1;
}

#define JS_INDENT            JS_Indent
#define JS_BRACE_OFS         JS_BraceOfs
#define JS_PAREN_DELTA       JS_ParenDelta
#define JS_CASE_OFS          JS_CaseOfs
#define JS_CASE_DELTA        JS_CaseDelta
#define JS_CLASS_OFS         JS_ClassOfs
#define JS_CLASS_DELTA       JS_ClassDelta
#define JS_COLON_OFS         JS_ColonOfs
#define JS_COMMENT_OFS       JS_CommentOfs
#define JS_COMMENT_DELTA     JS_CommentDelta
#define JS_CONTINUATION      JS_Continuation
#define JS_FIRST_INDENT      JS_FirstLevelIndent
#define JS_FIRST_WIDTH       JS_FirstLevelWidth
...
static int IndentNormal(EBuffer *B, int Line, int /*StateLen*/, hsState * /*StateMap*/) {
    ...
    if (LookAt(B, Line, 0, "case", hsC_Keyword) ||
        ...
        if (FindPrevIndent(B, RowP, ColP, CharP,
                           ...
                           FIND_ENDBLOCK) != 1) {
            if (RowP != PrevRowP)
                ContinuationIndent = JS_CONTINUATION;
            I = 0;
        ...
        switch (CharP) {
        case 'c':
            I = B->LineIndented(RowP) + JS_CONTINUATION;
            return I;

        case '(':
        case '[':
            if (JS_PAREN_DELTA >= 0) {
                I = B->LineIndented(FirstRowP) + JS_PAREN_DELTA;
            } else {
                ColP++;
        ...
        case '{':
            ...
            if (B->LineIndented(FirstRowP) <= JS_FIRST_WIDTH)
                I += JS_FIRST_INDENT;
            else
                I += JS_INDENT;
        ...
        case ':':
            ...
            case ':':
                ...
                /*if (SkipWhite(B, Line, RowP, ColP, SKIP_FORWARD) != 1)
                 return 0;
                 I = B->LineIndented(RowP);// - JS_COLON_OFS;
            ...
            case 'p':
                ...
                //if (SkipWhite(B, Line, RowP, ColP, SKIP_FORWARD) != 1)
                //                    return 0;
                I = B->LineIndented(RowP) + JS_CLASS_DELTA;
                //                if (FirstRowP == RowP) {
                //                    I += JS_CLASS_DELTA;
                ///                if (LookAt(B, Line, 0, "{", hsC_Normal, 0)) {
                ///                    I += JS_INDENT - JS_BRACE_OFS;
            ...
            case 'l':
                ColP++;
                I = B->LineIndented(RowP) + JS_BRACE_OFS;
                //C_CLASS_OFS + JS_CLASS_DELTA;
                break;
            case 'c':
                ColP++;
                //                if (SkipWhite(B, Line, RowP, ColP, SKIP_FORWARD) != 1)
                //                    return 0;
                I = B->LineIndented(RowP) + JS_CASE_DELTA;
                //                if (FirstRowP == RowP) {
                //                    I += JS_CASE_DELTA;
                ///                if (LookAt(B, Line, 0, "{", hsC_Normal, 0)) {
                ///                        I += JS_INDENT - JS_BRACE_OFS;
...
static int IndentComment(EBuffer *B, int Row, int /*StateLen*/, hsState * /*StateMap*/) {
    ...
    if (Row > 0) {
        ...
        if (B->RLine(Row - 1)->StateE == hsC_Comment)
            if (LookAt(B, Row - 1, I, "/*", hsC_Comment, 0)) I += JS_COMMENT_DELTA;
        if (B->RLine(Row - 1)->StateE == hsC_CPP_Comm)
            if (LookAt(B, Row - 1, I, "/*", hsC_CPP_Comm, 0)) I += JS_COMMENT_DELTA;
    }
    return I;
}
...
int Indent_JS(EBuffer *B, int Line, int PosCursor) {
    int I;
